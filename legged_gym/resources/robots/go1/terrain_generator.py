...
# 0 
        for k in range(4):
            local_pos = [1.9, 0.0, -0.5 * height]
            for i in range(stair_nums):
                local_pos[0] += width
                local_pos[2] += height
                x, y = rot2d(local_pos[0], local_pos[1], 1.57*k)
                self.AddBox([x + init_pos[0], y + init_pos[1], local_pos[2]],
                            [0.0, 0.0, 1.57*k], [width, length*(stair_nums-i)/stair_nums, height])

# 1
        init_pos=[4.0, 4.0, 0.0]
        for k in range(4):
            local_pos = [1.9, 0.0, -0.5 * height]
            for i in range(stair_nums):
                local_pos[0] += width
...
if __name__ == "__main__":
    ...
    # Stairs
    tg.AddStairs(init_pos=[0.0, 0.0, 0.0],height=0.16, yaw=0.0)
