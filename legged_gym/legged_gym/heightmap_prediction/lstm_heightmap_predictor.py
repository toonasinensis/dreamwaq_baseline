...
class LSTMHeightmapPredictor(nn.Module):
  ...
  def __init__(self,
    ...
    self.dim_n_proprio_hist_out = 16

    self.memory = Memory(self.dim_embedding +self.dim_n_proprio_hist_out,
    ...
    activation = nn.ELU()
    self.his_prop_mlp = nn.Sequential(
                                        nn.Linear(dim_n_proprio_hist_in, 128),
                                        activation,
                                        nn.Linear(128, 128),
                                        activation,
                                        nn.Linear(128, 16),
  ...
  def forward(self,
              ...
              proprio_hist,
              hidden_states=None):
    out = self.his_prop_mlp(proprio_hist)
    ...
    return self.forward_normalized(out,
  ...
  def train_on_data(self, replay_buffer, batch_size: int, num_steps: int):
    ...
    for _ in pbar:
      ...
      for batch in dataloader:
        optimizer.zero_grad()
        output = self.forward(
          batch['depth_imgs'],batch['base_states'])
